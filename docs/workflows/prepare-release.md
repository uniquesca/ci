...
database dump and a test coverage badge.
...
8. PHPUnit runs if the repository has a configuration, and the coverage badge is generated when
   asked for.
...
Staged explicitly: `package.json`, `composer.json`, the dump, `CHANGELOG.md` and the badge, each only
if the corresponding input asked for it. Then `git clean -fd`, which **removes untracked files a
`pre_cmd`, `post_cmd` or the dependency install left behind** - build output, in other words, does
not end up in the release commit.
