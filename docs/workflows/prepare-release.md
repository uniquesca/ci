...
| `generate_coverage_report` | boolean | `false` | Generate and commit a Markdown coverage report |
| `coverage_report_dir` | string | `docs/coverage-report` | Report directory. Ignored unless `generate_coverage_report` |
...
`generate_coverage_report` needs the PHPUnit configuration too. The report is
`coverage_report_dir/README.md`, built from PHPUnit's Clover output, so Github renders it when the
directory is opened: totals, then each file's line and method coverage and its uncovered lines.
`coverage_report_dir` is emptied before it is written, so nothing else can live in it,
